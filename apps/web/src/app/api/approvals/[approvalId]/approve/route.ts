...
export async function POST(_request: Request, context: { params: Promise<{ approvalId: string }> }): Promise<Response> {
  ...
  try {
    const upstream = await fetch(`${getApiBaseUrl()}${scopedApiPath(`approvals/${approvalId}/approve`)}`, { method: "POST", headers: { Accept: "application/json" }, signal: AbortSignal.timeout(120000) });
