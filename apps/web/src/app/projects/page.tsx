...
export default async function ProjectsPage() {
  ...
  return <div className="page-stack">
    <section className="page-heading" aria-labelledby="projects-title"><p className="eyebrow">Workspace</p><div><h1 id="projects-title">Projects</h1><p>Select an existing project or create one inside the configured local workspace.</p></div></section>
    <ProjectCreateForm />
    {projects === null ? <StatePanel title="Projects unavailable">The project service could not be reached or returned an invalid response. Try again when the API is available.</StatePanel> : projects.length === 0 ? <StatePanel title="No projects yet">Create the first project above to begin an operator workflow.</StatePanel> : <section aria-label="Projects" className="record-grid">{projects.map((project) => { const description = preview(project.description); return <article className="record-card" key={project.id}><div className="record-card-heading"><div><h2>{project.name}</h2><span className="record-slug">{project.slug}</span></div><StatusBadge tone={tones[project.status]}>{humanize(project.status)}</StatusBadge></div>{description ? <p className="record-description">{description}</p> : <p className="record-description record-description-empty">No description provided.</p>}<dl className="record-meta"><div><dt>Created</dt><dd><time dateTime={project.createdAt}>{formatDateTime(project.createdAt)}</time></dd></div><div><dt>Updated</dt><dd><time dateTime={project.updatedAt}>{formatDateTime(project.updatedAt)}</time></dd></div></dl><Link className="card-action" href={`/tasks?project=${project.id}`}>Open project tasks <span aria-hidden="true">→</span></Link></article>; })}</section>}
  </div>;
}
