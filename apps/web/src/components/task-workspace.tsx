...
import { GitHubPublication } from "@/components/github-publication";
...
function GitApproval({ action }: Readonly<{ action: GitCommitAction }>) { const [busy, setBusy] = useState(false); async function decide(decision: "approve" | "reject") { if (!action.approval_request_id) return; setBusy(true); try { await fetch(`/api/approvals/${action.approval_request_id}/${decision}`, { method: "POST" }); window.location.reload(); } finally { setBusy(false); } } return <>{action.status === "PENDING_APPROVAL" && action.approval_request_id ? <div className="challenge-actions"><button className="primary-button" disabled={busy} onClick={() => void decide("approve")}>Approve exact commit</button><button className="danger-button" disabled={busy} onClick={() => void decide("reject")}>Reject</button></div> : null}<GitHubPublication action={action} /></>; }
