...
const EVENT_TYPES = ["project.created", "task.created"] as const;
const MAX_EVENTS = 40;
type ConnectionState = "connecting" | "live" | "reconnecting" | "unavailable";
type RuntimeEvent = Readonly<{ id: string; eventType: string; source: string; occurredAt: string; projectId?: string; taskId?: string; summary: string }>;
...
function safeSummary(payload: unknown): string {
  ...
  for (const key of ["title", "name", "status", "state", "action"]) { const value = payload[key]; if (typeof value === "string" && value.trim()) return `${humanize(key)}: ${value.trim().slice(0, 120)}`; }
...
function parseEvent(message: MessageEvent<string>): RuntimeEvent | null {
  try {
    ...
    return { id: value.id, eventType: value.event_type, source: value.source, occurredAt: value.occurred_at, ...(uuid(value.project_id) ? { projectId: value.project_id } : {}), ...(uuid(value.task_id) ? { taskId: value.task_id } : {}), summary: safeSummary(value.payload) };
  } catch { return null; }
}
...
export function ActivityFeed() {
  ...
  return <section className="activity-panel" aria-labelledby="activity-feed-title"><div className="activity-header"><div><h2 id="activity-feed-title">Runtime events</h2><p>Newest first · up to {MAX_EVENTS} events retained in memory</p></div><StatusBadge tone={stateTone[connection]}>{humanize(connection)}</StatusBadge></div><div className="sr-status" role="status" aria-live="polite">Activity connection is {connection}.</div>{events.length === 0 ? <div className="activity-empty"><h3>{connection === "live" ? "Waiting for activity" : "Connecting to activity"}</h3><p>New project and task creation events will appear here without being stored in the browser.</p></div> : <ol className="activity-list">{events.map((event) => <li key={event.id}><div className="activity-marker" aria-hidden="true" /><article><div className="activity-event-heading"><h3>{humanize(event.eventType.replace(".", " "))}</h3><time dateTime={event.occurredAt}>{formatDateTime(event.occurredAt)}</time></div><p>{event.summary}</p><dl className="activity-meta"><div><dt>Source</dt><dd>{event.source}</dd></div>{event.projectId ? <div><dt>Project</dt><dd title={event.projectId}>{event.projectId.slice(0, 8)}</dd></div> : null}{event.taskId ? <div><dt>Task</dt><dd title={event.taskId}>{event.taskId.slice(0, 8)}</dd></div> : null}</dl></article></li>)}</ol>}</section>;
}
